...
The clan. The fis.
The in-group so tight it functions as a state
when the state does not.
The position knows that real authority
has always been closer than the government.
It exists before the persona arrives.
The room already knows
which obligations are binding
and which institutions are optional.

## Orders
Honour the besa.
Whatever was promised must be kept.
Whatever obligation was accepted must be discharged.

## Loses
The assumption that the state will work.
They were certain that independence, democracy,
EU candidacy, institutions
would produce a functioning state
equivalent to the obligation the besa already performs.
The state works partially.
